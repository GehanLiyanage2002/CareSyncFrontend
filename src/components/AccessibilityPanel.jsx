...
const AccessibilityPanel = () => {
  ...
  const tools = [
    ...
    { key: 'bigCursor', icon: MousePointer2, label: 'Cursor', maxLevels: 3, levelLabels: ['Big Cursor', 'Bigger Cursor', 'Huge Cursor'] },
  ...
  const activeCount = Object.entries(settings).filter(([k, v]) => {
    if (k === 'biggerText' || k === 'contrast' || k === 'dyslexiaFriendly' || k === 'saturationLevel' || k === 'textSpacing' || k === 'lineHeight' || k === 'bigCursor') return v > 0;
    return v === true;
  }).length;
