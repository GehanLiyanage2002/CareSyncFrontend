...
const AdminServices = () => {
  ...
  const fetchSchedules = async (serviceId) => {
    try {
      const res = await axios.get(`http://127.0.0.1:5000/api/services/${serviceId}/schedules`, {
        headers: { Authorization: token }
      });
      if (res.data.success) {
  ...
  const uploadAvatar = async (serviceId) => {
    ...
    try {
      await axios.put(`http://127.0.0.1:5000/api/services/${serviceId}/image`, fd, {
  ...
  const handleSaveInfo = async (e) => {
    ...
    try {
      if (isEditing) {
        const res = await axios.put(`http://127.0.0.1:5000/api/services/${currentService.id}`, {
        ...
        const res = await axios.post('http://127.0.0.1:5000/api/services', {
          name: currentService.name,
          description: currentService.description,
          location: currentService.location,
          price: 0
  ...
  const handleAddSchedule = async (e) => {
    ...
    try {
      if (newSchedule.type === 'date') {
        ...
        await axios.post(`http://127.0.0.1:5000/api/services/${currentService.id}/schedules`, payload, {
          headers: { Authorization: token }
        });
      } else {
        const promises = newSchedule.day_of_week.map(day => {
          return axios.post(`http://127.0.0.1:5000/api/services/${currentService.id}/schedules`, {
            start_time: newSchedule.start_time,
            end_time: newSchedule.end_time,
            day_of_week: day
          }, { headers: { Authorization: token } });
  ...
  const handleDeleteSchedule = async (scheduleId) => {
    try {
      const res = await axios.delete(`http://127.0.0.1:5000/api/services/schedules/${scheduleId}`, {
        headers: { Authorization: token }
      });
      if (res.data.success) {
  ...
  const handleEdit = (service) => {
    ...
    if (service.has_image) {
      setAvatarPreview(`http://127.0.0.1:5000/api/services/${service.id}/image?t=${Date.now()}`);
    } else {
  ...
  const toggleAvailability = async (id, currentStatus) => {
    try {
      ...
      const res = await axios.put(`http://127.0.0.1:5000/api/services/${id}`, {
        name: service.name,
        description: service.description,
        location: service.location,
        price: service.price,
  ...
  const confirmDeleteService = async () => {
    ...
    try {
      const res = await axios.delete(`http://127.0.0.1:5000/api/services/${serviceToDelete.id}`, {
        headers: { Authorization: token }
      });
      if (res.data.success) {
                ...
                <div className="flex justify-between items-start">
                  <div className="flex gap-4">
                    <div className="w-[70px] h-[70px] rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-700 font-bold overflow-hidden border border-indigo-100 flex-shrink-0 shadow-inner">
                      <img 
                        src={`http://127.0.0.1:5000/api/services/${service.id}/image?t=${Date.now()}`} 
                        alt={service.name} 
            ...
            <div className="px-8 pb-8">
              <div className="flex justify-between items-start mb-6">
                <div className="w-24 h-24 rounded-2xl bg-white border-4 border-white shadow-lg overflow-hidden flex-shrink-0 flex items-center justify-center text-teal-700 font-bold text-3xl -mt-12 relative z-10">
                  <img src={`http://127.0.0.1:5000/api/services/${selectedService.id}/image?t=${Date.now()}`} alt={selectedService.name} className="w-full h-full object-cover" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; if(e.target.parentNode) e.target.parentNode.innerText = selectedService.name.charAt(0).toUpperCase(); }} />
                </div>
