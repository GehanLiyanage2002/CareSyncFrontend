...
const ActivityIcon = ({ className = "w-5 h-5" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M11.99 7.5v.008M12 7.5a.75.75 0 110 1.5.75.75 0 010-1.5zM12 21a9 9 0 100-18 9 9 0 000 18zM12.03 16.5v-6" />
  </svg>
);
...
const PatientDashboard = () => {
  ...
  const navItems = [
    ...
    { name: 'Medical Services', icon: <ActivityIcon /> },
    { name: 'My Reviews', icon: <StarIcon /> },
    { name: 'Medical Profile', icon: <UserIcon /> },
    { name: 'Settings', icon: <CogIcon /> },
  ];

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-900 flex flex-col md:flex-row font-sans text-slate-800 dark:text-white selection:bg-blue-100">
      
      {/* Sidebar */}
      <aside className="w-full md:w-72 bg-white dark:bg-gray-800 border-r border-slate-200 dark:border-gray-600 shadow-sm flex flex-col justify-between shrink-0">
        <div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-xl transition-all duration-200 font-semibold ${
                  activeTab === item.name
                    ...
                    : 'text-slate-500 dark:text-gray-400 hover:bg-slate-50 dark:bg-gray-900 hover:text-blue-600'
                }`}
              >
                {item.icon}
                {item.name}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-gray-700 flex flex-col gap-2">
          <button
            onClick={() => navigate('/edit-profile')}
            className="w-full flex items-center gap-3 px-5 py-3 text-slate-500 dark:text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-200 font-semibold"
          >
            <UserIcon />
            Edit Profile
          </button>
          <button
            ...
            className="w-full flex items-center gap-3 px-5 py-3 text-slate-500 dark:text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-200 font-semibold"
          >
            <LogoutIcon />
            Log Out
          </button>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-slate-100 dark:border-gray-700 p-8 min-h-[400px]">
          {activeTab === 'Medical Profile' ? (
            ...
            <>
              <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-6">{activeTab} Overview</h3>
              <div className="flex flex-col items-center justify-center h-64 text-slate-400 bg-slate-50 dark:bg-gray-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-gray-600">
