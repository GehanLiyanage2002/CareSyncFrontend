...
const ReceptionistKanbanBoard = ({ allAppointments = [], doctor }) => {
 ...
 return (
 <div key={column.id} className="flex flex-col bg-slate-50 dark:bg-gray-900/50 /50 rounded-3xl border border-slate-100 dark:border-gray-700 overflow-hidden shadow-sm h-full">
 ...
 className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-slate-100 dark:border-gray-700 shadow-sm cursor-pointer hover:shadow-md hover:border-blue-200 :border-blue-600 transition-all"
 >
 <div className="flex justify-between items-start mb-4">
 <div>
 <span className="inline-flex items-center gap-1 px-2.5 py-1 mb-3 text-[10px] font-black uppercase tracking-widest bg-slate-100 dark:bg-gray-800 text-slate-500 dark:text-gray-400 rounded-lg">
 <Hash size={10} />
 ...
 <span className="flex items-center gap-1.5 text-slate-600 dark:text-gray-300 bg-slate-50 dark:bg-gray-900 px-2.5 py-1.5 rounded-lg border border-slate-100 dark:border-gray-700 ">
 <Calendar size={12} />
 ...
 >
 <div className="p-6 border-b border-slate-100 dark:border-gray-700 flex justify-between items-center bg-gradient-to-r from-blue-50 to-indigo-50 ">
 <h3 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2">
 <User className="text-blue-600" /> Patient Info
 </h3>
 <button 
 onClick={() => setSelectedTask(null)}
 className="p-2 text-slate-400 hover:text-slate-800 dark:text-white :text-white rounded-full hover:bg-white dark:bg-gray-800 :bg-gray-700 shadow-sm transition-all"
 >
 <X size={20} />
 </button>
 </div>
 ...
 <div className="col-span-2 pt-4 border-t border-slate-200 dark:border-gray-600 ">
 ...
 className="bg-white dark:bg-gray-800 rounded-[2rem] max-w-md w-full max-h-[95vh] overflow-y-auto shadow-2xl border border-gray-100 /80 animate-in zoom-in-95 duration-200 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
 onClick={(e) => e.stopPropagation()}
 >
 <div className="bg-gradient-to-r from-emerald-600 to-teal-500 p-5 text-center text-white relative print:bg-white dark:bg-gray-800 print:text-black dark:text-white print:border-b print:border-gray-200 dark:border-gray-700 print:from-white print:to-white">
 <div className="w-12 h-12 bg-white dark:bg-gray-800 /20 backdrop-blur-md rounded-full flex items-center justify-center mx-auto mb-3 border border-white/30 animate-bounce print:animate-none print:bg-gray-100 print:border-gray-300 dark:border-gray-600">
 <CheckCircle className="h-6 w-6 text-white print:text-emerald-600" />
 </div>
 <h4 className="text-xl font-black print:text-slate-800 dark:text-white ">Billing Receipt</h4>
 <p className="text-xs text-emerald-100 font-bold uppercase tracking-widest mt-1 print:text-slate-500 dark:text-gray-400 ">CareSync Consultation Bill</p>
 </div>

 <div className="p-6 space-y-4">
 <div className="bg-teal-50 /20 border border-teal-100 /60 rounded-2xl py-3 px-4 text-center shadow-sm print:shadow-none print:border-gray-200 dark:border-gray-700 print:bg-gray-50 dark:bg-gray-900">
 <span className="block text-[10px] text-teal-600 font-black uppercase tracking-widest mb-1 print:text-gray-500 dark:text-gray-400">Queue Token</span>
 <span className="text-3xl font-black text-teal-800 tracking-wider font-mono print:text-black dark:text-white">{billingTask.tokenNumber || 'N/A'}</span>
 </div>

 <div className="border-2 border-dashed border-gray-100 p-5 rounded-3xl space-y-3 bg-gray-50/50 /20 print:border-gray-300 dark:border-gray-600 print:bg-white dark:bg-gray-800 print:rounded-xl">
 {doctor && (
 <>
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Doctor</span>
 <span className="text-gray-800 dark:text-white font-black text-base print:text-black">Dr. {doctor.name || doctor.doctor_name}</span>
 </div>
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Specialization</span>
 <span className="text-gray-800 dark:text-white font-bold print:text-black">{doctor.specialization || 'General'}</span>
 </div>
 </>
 )}
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Patient Name</span>
 <span className="text-gray-800 dark:text-white font-black text-base print:text-black">{billingTask.patientName}</span>
 </div>
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Contact</span>
 <span className="text-gray-800 dark:text-white font-bold print:text-black">{billingTask.contact || 'N/A'}</span>
 </div>
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Date & Time</span>
 <span className="text-gray-800 dark:text-white font-bold print:text-black">{billingTask.date} @ {billingTask.time}</span>
 </div>
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Type</span>
 <span className="text-gray-800 dark:text-white font-bold print:text-black">{billingTask.type}</span>
 </div>
 <div className="flex justify-between items-center text-sm">
 <span className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Payment</span>
 <span className="text-emerald-600 font-black print:text-black dark:text-white">
 ...
 <span className="text-gray-500 dark:text-gray-400 font-black uppercase tracking-widest text-xs">Amount Due</span>
 ...
 className="flex-1 border border-slate-200 dark:border-gray-600 hover:bg-slate-50 dark:bg-gray-900 :bg-gray-800 text-slate-700 dark:text-gray-200 font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all text-sm shadow-sm hover:shadow-md hover:scale-[1.02] active:scale-[0.98]"
 >
 ...
 className="flex-1 bg-slate-900 hover:bg-slate-800 :bg-gray-100 dark:bg-gray-800 text-white font-bold py-3 rounded-2xl transition-all text-sm shadow-sm hover:shadow-md hover:scale-[1.02] active:scale-[0.98]"
 >
 Close
 </button>
 </div>
