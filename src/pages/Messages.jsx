...
const Messages = () => {
  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:5000/api/contact-messages', config);
  ...
  const handleMarkAsRead = async (id) => {
    try {
      const response = await axios.put(`http://127.0.0.1:5000/api/contact-messages/${id}/read`, {}, config);
      if (response.data.success) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axios.delete(`http://127.0.0.1:5000/api/contact-messages/${messageToDelete}`, config);
      if (response.data.success) {
  ...
  const submitReply = async () => {
    ...
    setReplying(true);
    try {
      const response = await axios.post(`http://127.0.0.1:5000/api/contact-messages/${messageToReply.id}/reply`, {
        replyText,
