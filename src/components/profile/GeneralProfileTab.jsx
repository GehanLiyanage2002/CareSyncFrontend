...
const GeneralProfileTab = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/users/profile-image`, formData, {
        headers: { 
          Authorization: token,
  ...
  const handleFaceCapture = async (faceDescriptor) => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/users/face-id`,
        { faceDescriptor },
        {
          headers: {
            Authorization: token,
  ...
  useEffect(() => {
    const fetchDoctorProfile = async () => {
      if (user?.role === 'Doctor') {
        try {
          const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/users/doctor-profile`, {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/users/general`, {
        full_name: formData.fullName,
        mobile_number: formData.contactNumber
      }, {
        headers: { Authorization: token }
  ...
  const handleSavePassword = async (e) => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/users/password`, passwordData, {
        headers: { Authorization: token }
      });
  ...
  const handleSaveDoctorProfile = async (e) => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/users/doctor-profile`, doctorData, {
        headers: { Authorization: token }
      });
          ...
          <div className="flex items-center gap-6 pb-6 border-b border-slate-100 dark:border-slate-700">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              <div className="w-24 h-24 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-600 dark:text-blue-300 text-3xl font-bold shadow-inner overflow-hidden border-2 border-white dark:border-slate-800">
                <img 
                  src={user?.profile_image || `${import.meta.env.VITE_API_URL}/api/users/profile-image/${user?.id}?t=${Date.now()}`}
                  alt="Profile" 
