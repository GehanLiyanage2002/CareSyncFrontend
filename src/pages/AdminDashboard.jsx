...
const AdminDashboard = () => {
 ...
 try {
 const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/appointments/${appointmentToCancel}/cancel`, {}, config);
 if (res.data.success) {
 ...
 try {
 let url = `${import.meta.env.VITE_API_URL}/api/admin/stats`;
 ...
 try {
 const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/doctors`, config);
 ...
 try {
 const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/services/bookings`, config);
 ...
 try {
 const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/patients`, config);
 ...
 try {
 const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/appointments`, config);
 ...
 try {
 let url = `${import.meta.env.VITE_API_URL}/api/admin/earnings`;
 const { startDate, endDate } = getFilterDates();
 if (startDate && endDate) {
 url += `?startDate=${startDate}&endDate=${endDate}`;
 }
 ...
 const handleApproveDoctor = async (id, currentStatus) => {
  ...
  setIsApproving(true);
  try {
  const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/doctors/${id}/approve`, {
 ...
 try {
 const res = await axios.delete(`${import.meta.env.VITE_API_URL}/api/admin/doctors/${doctorToDelete.id}`, config);
 toast.success(res.data.message);
 ...
 <img 
 src={`${import.meta.env.VITE_API_URL}/api/users/profile-image/${earn.doctor_id}`} 
 ...
 <img src={`${import.meta.env.VITE_API_URL}/api/users/profile-image/${doctor.id}`} alt={doctor.full_name} className="w-full h-full object-cover" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; e.target.parentNode.innerHTML = doctor.full_name.charAt(0); }} />
 </div>
       ...
       <div className="flex flex-col items-center gap-2">
         ...
         <div className="w-full h-32 rounded-xl border border-slate-200 dark:border-gray-600 overflow-hidden shadow-sm cursor-pointer">
           <AuthorizedImage 
             ...
             token={token} 
             alt="ID Card Front" 
             onClick={() => setEnlargedImage(`${import.meta.env.VITE_API_URL}/api/admin/doctors/${selectedDoctor.id}/id-card/front`)}
           />
         </div>
       </div>
     ) : (
       ...
       <div className="flex flex-col items-center gap-2">
         ...
         <div className="w-full h-32 rounded-xl border border-slate-200 dark:border-gray-600 overflow-hidden shadow-sm cursor-pointer">
           <AuthorizedImage 
             ...
             token={token} 
             alt="ID Card Rear" 
             onClick={() => setEnlargedImage(`${import.meta.env.VITE_API_URL}/api/admin/doctors/${selectedDoctor.id}/id-card/rear`)}
           />
         </div>
       </div>
     ) : (
